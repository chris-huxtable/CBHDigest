...
#define CBHAssertDigest(digest, expected)\
{ \
	NSString *digestString = [digest encodeAsHexadecimal];\
	\
	...
	\
	digestString = [digest encodeAsCapitalizedHexadecimal];\
	XCTAssertEqualObjects(digestString, [expected uppercaseString], @"Digest should match known digest.");\
}
